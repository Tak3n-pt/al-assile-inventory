...
const QuoteTemplate = forwardRef(({ data, settings, language = 'fr' }, ref) => {
  const {
    number,
    date,
    client,
  ...
  const totalTTC = (subtotal || 0) + tvaAmount - (discount || 0);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-5">
        <div className="text-9xl font-bold text-violet-600 rotate-[-30deg]">DEVIS</div>
      </div>
      ...
      <div className="flex justify-between items-start mb-8 border-b-2 border-violet-600 pb-6 relative">
        ...
        <div className="text-right">
          <div className="bg-violet-600 text-white px-6 py-3 rounded-lg inline-block">
            ...
            <p className="text-violet-100 text-sm">عرض أسعار</p>
          </div>
          <div className="mt-4 text-right">
            <p className="text-gray-500 text-xs">N° Devis:</p>
      ...
      <div className="bg-violet-50 border border-violet-200 rounded-lg p-4 mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-violet-600 font-bold">{addDays(date, validityDays)}</p>
        </div>
      ...
      {/* Totals: subtotal / discount / TVA / total TTC */}
      <div className="flex justify-end mb-6">
        <div className="w-80">
          ...
          <div className="flex justify-between py-3 bg-violet-600 text-white px-4 rounded-lg mt-2">
            ...
            <span className="font-bold font-mono">{formatCurrency(total || totalTTC)} DA</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 p-4 rounded-lg mb-6">
        <h4 className="font-semibold text-amber-800 mb-2">Conditions de paiement / شروط الدفع:</h4>
        <p className="text-gray-700 text-sm">
          {paymentTerms || 'Paiement à la commande. Conditions à confirmer lors de l\'acceptation du devis.'}
        </p>
      </div>

      {/* General Conditions */}
      <div className="bg-gray-50 border border-gray-200 p-4 rounded-lg mb-6">
        <h4 className="font-semibold text-gray-700 mb-2">Conditions générales / الشروط العامة:</h4>
        <p className="text-gray-600 text-xs leading-relaxed">
          {generalConditions ||
            'Les prix indiqués sont hors taxes. La TVA sera appliquée au taux en vigueur. ' +
            'Toute commande passée suite à ce devis implique l\'acceptation de nos conditions générales de vente. ' +
            'Les délais de livraison seront confirmés à la réception de la commande ferme.'}
        </p>
      </div>
      ...
      <div className="bg-gray-100 p-4 rounded-lg mb-6 text-center">
        <p className="text-sm text-gray-600 font-medium">
          Ce document n'a pas valeur de facture. Il s'agit d'une offre de prix soumise à acceptation.
        </p>
        <p className="text-xs text-gray-500 mt-1" dir="rtl">
          هذه الوثيقة ليست فاتورة رسمية. إنها عرض أسعار يخضع للقبول.
        </p>
      </div>
      ...
      <div className="mt-8 pt-4 border-t border-gray-300 flex justify-between items-end">
        <div className="text-xs text-gray-500">
          ...
          <p>Devis valable jusqu'au {addDays(date, validityDays)}</p>
        </div>
